...
@app.route('/api/pressure/<variable>/<start>/<end>', methods=['GET']) 
def get_pressure_mmar(start,end):   
    '''RETURNS MIN, MAX, AVG AND RANGE FOR PRESSURE. THAT FALLS WITHIN THE START AND END DATE RANGE'''
   
    if request.method == "GET": 
        '''Add your code here to complete this route'''
        if mongo.pressureMMAR(escape(start),escape(end)):
            return jsonify({"status":"found","data":(mongo.pressureMMAR(escape(start),escape(end)))})
    # FILE DATA NOT EXIST
    return jsonify({"status":"not found","data":[]})
...
def get_heatindex_mmar(start,end):   
    ...
    if request.method == "GET": 
        '''Add your code here to complete this route'''
        if mongo.heatIndexMMAR(escape(start),escape(end)):
            return jsonify({"status":"found","data":(mongo.heatIndexMMAR(escape(start),escape(end)))})
    # FILE DATA NOT EXIST
    return jsonify({"status":"not found","data":[]})
...
def get_altitude_mmar(start,end):
    ...
    if request.method == "GET":
        '''Add your code here to complete this route'''
        if mongo.altitudeMMAR(escape(start),escape(end)):
            return jsonify({"status":"found","data":(mongo.altitudeMMAR(escape(start),escape(end)))})
    # FILE DATA NOT EXIST
    return jsonify({"status":"not found","data":[]})
...
def get_moisture_mmar(start,end):
    '''RETURNS MIN, MAX, AVG AND RANGE FOR MOISTURE. THAT FALLS WITHIN THE START AND END DATE RANGE'''

    if request.method == "GET":
        '''Add your code here to complete this route'''
        if mongo.moistureMMAR(escape(start),escape(end)):
            return jsonify({"status":"found","data":(mongo.moistureMMAR(escape(start),escape(end)))})
    # FILE DATA NOT EXIST
    return jsonify({"status":"not found","data":[]})
